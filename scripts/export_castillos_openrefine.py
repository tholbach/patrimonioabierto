#!/usr/bin/env python3
"""OpenRefine-ready export of every CASTILLOS entry, for reconciling/
creating the missing Wikidata items one JCyL id at a time - same approach
as export_arte_rupestre_openrefine.py (one item per JCyL id, no shared/
"part of" grouping). Unlike arte rupestre, find_bic_clusters.py found only
2 tiny candidate clusters for this category - not a real clustering
question here, so there's nothing to second-guess before starting.

Same shape as export_arte_rupestre_openrefine.py (lat_lon combined into
one column, ready to reconcile against in OpenRefine or feed into a
"coordinate location" column), just filtered to this category instead.

Includes the already-linked entries too (not just the missing ones) so
they're visible for context in OpenRefine rather than silently absent -
skip those rows rather than accidentally creating a duplicate item.

Run: python3 scripts/export_castillos_openrefine.py
Writes: data/raw/castillos_openrefine.csv (gitignored, like the rest of
...
OUT_PATH = os.path.join(RAW_DIR, "castillos_openrefine.csv")
